...
onMounted(() => {
  axios({
    method: 'get',
    url: `http://127.0.0.1:8000/api/deposits/saving-product/${props.savingProduct.fin_prdt_cd}`
  })
  .then((res) => {
    item.value = res.data
  })
  ...
  .then((res) => {
    ...
    if (props.savingProduct.join_way !== undefined && (props.savingProduct.join_way.includes('인터넷') || (props.savingProduct.join_way.includes('스마트폰'))) ) {
      remote.value = '방문없이가입'
    }
